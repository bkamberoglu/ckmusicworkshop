...
from flask import Flask, render_template, current_app, request, redirect,jsonify, url_for, flash, abort, g
from flask import session as login_session
from flask import make_response

#from recaptcha import check, confirm

from sqlalchemy import asc, desc, func, exc
from sqlalchemy.sql import and_, or_
from sqlalchemy.sql.expression import true

from .models import Student, Address, Payment, MusicalNote, StudentAttendHistory, User, StudentMusicalNoteLink
...
from app import app, moment, db, login_manager, GOOGLE_CLIENT_ID
...
def before_request():
    ...
    if g.user.is_authenticated:
      ...
      db.session.commit()
    if g.user != None and g.user.is_anonymous:
      g.user = None

'''
@app.before_request
def make_session_modified():
  print ("I am in make_session_modified =======> ")
...
def make_session_permanent():
  ...
  # Set the timeout for our session to two hours
  # The session will be lost after two hours with no interaction
  # form the user.
...
def showActiveStudents():
  ...
  if 'username' in login_session:
    if login_session['email'] in current_app.config['ADMIN_EMAILS']:
  ...
  return render_template('activeStudents.html', students = students, currentUser = g.user)
...
def showInactiveStudents():
  if 'username' in login_session:
    ...
    return render_template('inactiveStudents.html', students = students, currentUser = g.user)
  else:
...
def showAllUsers():
  if 'username' in login_session:
    if login_session['email'] in current_app.config['ADMIN_EMAILS']:
      ...
      users = [user for user in User.query.order_by(asc(User.numberOfTimesLoggedIn)).all()]
      current_app.logger.info('Displaying all logged in users.')

      #users = db.session.query(User).order_by(asc(User.numberOfTimesLoggedIn)).all()
      return render_template('userInfo.html', users = users, currentUser = g.user)
  return redirect(url_for('showActiveStudents')) 
...
def newStudent():
  ...
  if request.method == 'POST':      
      try:
        ...
        newStudentAttendHistory = StudentAttendHistory(student = newStudent)

        db.session.add(newStudentAttendHistory)
        db.session.commit()
        current_app.logger.info('New Student %s Successfully Created' % newStudent.id)
...
def editStudent(id):
    editStudent = Student.query.get_or_404(id)
    #db.session.query(Student).filter_by(id = student_id).one()
    ...
    studentAttendHistories = StudentAttendHistory.query.filter_by(student_id = id).all()
    #db.session.query(StudentAttendHistory).filter_by(student_id = id).all()
    if request.method == 'POST':  
      try:    
        ...
        if currentUser.isAdmin:
          if editStudent.isActive == True and request.form.get('isActive') == None:
            ...
            studentAttendHistory = StudentAttendHistory.query.filter_by(student_id = id, endDate = None).first_or_404()
            #db.session.query(StudentAttendHistory).filter_by( student_id = id, endDate = None).one()
            studentAttendHistory.endDate = datetime.utcnow().strftime("%Y-%m-%d")
          ...
          if editStudent.isActive == False and bool(request.form.get('isActive')) == True:
            ...
            newStudentAttendHistory = StudentAttendHistory(student_id = editStudent.id)
            db.session.add(newStudentAttendHistory)
        db.session.commit()
        current_app.logger.info('Student Successfully Edited %s' % editStudent)
...
def addPayment(student_id):
    if 'username' in login_session:    
      ...
      if request.method == 'POST':
        ...
        return render_template('payment.html', student = student, payments = payments, currentUser = g.user)
    else:
      return redirect(url_for('showActiveStudents'))
...
def showMyNotes(student_id, displayAll):
  if 'username' in login_session:
    ...
    currentUser = g.user
    #getUserByEmail(login_session['email'])
...
def deleteMyNote(student_id, musicalNote_id):
  ...
  db.session.delete(studentMusicalNoteLink)
...
def deleteNote(musicalNote_id):
  ...
  db.session.commit()
  current_app.logger.info('%s is deactivated.', musicalNote)
...
def showNotes():
  ...
  if request.method == 'POST':
    ...
    current_app.logger.info('All %s active note(s) is/are displayed.', len(musicalNotes))
    return render_template('notes.html', musicalNotes = musicalNotes, currentUser = g.user)
...
def showInactiveNotes():
  if 'username' in login_session:
    ...
    current_app.logger.info('All %s inactive note(s) is/are displayed.', len(musicalNotes))
    return render_template('inactivenotes.html', musicalNotes = musicalNotes, currentUser = g.user)
  else:
    return redirect(url_for('showNotes'))
...
def contactUs():
  grecaptchaSitekey = current_app.config['RECAPTCHA_SITEKEY']
  print ("contactus current_app.config['MAIL_USERNAME'] ====> ", current_app.config['MAIL_USERNAME'])
  print ("contactus current_app.config['RECAPTCHA_SITEKEY'] ====> ", current_app.config['RECAPTCHA_SITEKEY'])
  if request.method == 'POST':
    ...
    text = msg.as_string()
    try:  
      server.sendmail(current_app.config['MAIL_USERNAME'], current_app.config['MAIL_TO'], text)
      #flash ('Contact form successfully submitted. Thank you, I will get back to you soon!')
      flash('Thank you!' + flask.Markup('<br />') + 
    ...
    return render_template('contactForm.html', currentUser = g.user, grecaptchaSitekey = grecaptchaSitekey) 
  else:
    return render_template('contactForm.html', currentUser = g.user, grecaptchaSitekey = grecaptchaSitekey)
...
def activedNote(musicalNote_id):
  ...
  db.session.add(musicalNote)
  db.session.commit()
  current_app.logger.info('%s is activated.', musicalNote)
  flash('%s Successfully Activated' % musicalNote)
...
def disconnect():
    if 'provider' in login_session:
      ...
      logout_user()
      current_app.logger.info('%s - %s - %s logged out.', user_id, \
        username, email)
...
def fbconnect():
    ...
    #print("current_app.config.get('FB_APP_ID')=====> ", current_app.config.get('FB_APP_ID'))
    #print("current_app.config.get('FB_APP_SECRET') =====> ", current_app.config.get('FB_APP_SECRET'))

    print ("current_app.config.get['FB_APP_ID'] ===> ", current_app.config['FB_APP_ID'])
    print ("current_app.config.get['FB_APP_SECRET'] =====> ", current_app.config['FB_APP_SECRET'])

    url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s' % (
      current_app.config['FB_APP_ID'], current_app.config['FB_APP_SECRET'], access_token)
...
def method_not_found_error(error):
    app.logger.error('Method Not Found: %s', (error))
